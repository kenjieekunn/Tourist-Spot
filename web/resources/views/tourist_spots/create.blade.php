            ...
            <div class="card-body p-4">
                ...
                <form id="tourist-spot-form" action="{{ route('tourist_spots.store') }}" method="POST" enctype="multipart/form-data">
                    ...
                    <div class="mb-3">
                        ...
                        <label for="images" class="dropzone d-block"><i class="fas fa-cloud-arrow-up d-block mb-2"></i><strong>Drag & drop photos here or click to browse</strong><small class="d-block text-muted mt-1">At least 1 reference image required, up to 5 JPG, PNG, or WebP images, 2MB each</small></label>
...
<script>
    ...
    function geocodeLocationQuery(query) {
        ...
        const fallbackToNominatim = () => searchNominatimSuggestions(normalizedQuery).then((results) => {
    ...
    async function selectSuggestion(result) {
        ...
        if (result.type === 'google' && googleGeocoder && result.placeId) {
            try {
                ...
                if (place) {
                    ...
                    if (!isLocationWithinMunicipality(lat, lng)) {
                        alert('The selected location is outside the assigned municipality.');
                        return;
                    }
        ...
        const lng = Number(result.longitude ?? result.lng ?? result.lon);
