...
<script>
    ...
    function isLocationWithinMunicipality(lat, lng) {
        const boundary = districtMapContext?.boundary;
        if (Array.isArray(boundary) && boundary.length > 2) {
            ...
            for (let index = 0, previous = boundary.length - 1; index < boundary.length; previous = index++) {
                const [currentLng, currentLat] = boundary[index];
                const [previousLng, previousLat] = boundary[previous];
        ...
        if (googleGeocoder && window.google?.maps) {
            return new Promise((resolve) => {
                googleGeocoder.geocode({
                    ...
                    fallbackToNominatim().then(resolve);
                });
            });
        }
    ...
    function getExactBarangayResult(query) {
        const normalizedQuery = normalizeSearchText(query).replace(/^barangay\s+/, '');
        ...
        return match ? {
            ...
            subtitle: `Barangay - ${match.municipalityName}, Pangasinan 2nd District`,
            source: 'Barangay'
        } : null;
    }
        ...
        @if($spot)
            ...
            marker.addListener('dragend', function() {
                ...
                if (!isLocationWithinMunicipality(lat, lng)) {
                    marker.setPosition({ lat: Number(document.getElementById('latitude').value), lng: Number(document.getElementById('longitude').value) });
                    alert(`The selected location must be inside ${assignedMunicipalityName || 'the assigned municipality'}.`);
                    return;
                }
    ...
    function searchLocationAPI(query) {
        ...
        const exactBarangay = getExactBarangayResult(normalizedQuery);
        if (exactBarangay) {
            selectSuggestion(exactBarangay);
            setSearchStatus('Barangay location selected.');
            return;
        }
    ...
    async function selectSuggestion(result) {
        ...
        if (result.type === 'barangay') {
            ...
            if (place?.geometry?.location) {
                ...
                if (!isLocationWithinMunicipality(lat, lng)) {
                    alert(`The selected location is outside ${assignedMunicipalityName || 'the assigned municipality'}.`);
                    return;
                }
                updateCoordinates(lat, lng);
        ...
        const localLatitude = Number(result.latitude ?? result.lat);
        const localLongitude = Number(result.longitude ?? result.lng ?? result.lon);
        if (Number.isFinite(localLatitude) && Number.isFinite(localLongitude)) {
                if (!isLocationWithinMunicipality(localLatitude, localLongitude)) {
                    alert(`The selected location is outside ${assignedMunicipalityName || 'the assigned municipality'}.`);
                    return;
                }
            updateCoordinates(localLatitude, localLongitude);
            map.setCenter({ lat: localLatitude, lng: localLongitude });
            map.setZoom(17);
            ensureMarker(localLatitude, localLongitude);
            const localAddress = result.subtitle || result.name || '';
            const addressInput = document.getElementById('address');
            if (addressInput && localAddress) addressInput.value = localAddress;
            const addressPreview = document.getElementById('resolved-address-preview');
            if (addressPreview && localAddress) addressPreview.textContent = localAddress;
            return;
        }
