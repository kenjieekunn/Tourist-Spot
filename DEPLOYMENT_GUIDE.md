...
6. **Run Migrations**
   ...
   The repository root also includes `hostinger-build.sh`. Configure this as
   the Hostinger Git deployment build command, or run it from `public_html`:

   ```bash
   bash hostinger-build.sh
   ```

   This is required because `web/vendor/autoload.php` is generated by Composer
   and is intentionally not stored in GitHub.
